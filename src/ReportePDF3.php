<?php
	...
	//Código para incluir las librerias
     ...
     $result=mysql_query($sql,$link);
	   ...
	   $pdf->Cell(0,8,"empleados",0,0,'C',0);
	   $pdf->Cell(0,20,"",0,1,'',0);+
		...
		$pdf->Cell(50,5,'direccion',1,1,'C');
		$pdf->SetFont('Arial','',10);

		//impresion de datos obtenidos desde la BD
		 while($row = mysql_fetch_array($result)){
			  $pdf->Cell(50,5,$row["cargo"],1,0,'C');
			  ...
			  $pdf->Cell(50,5,$row["edad"],1,0,'C');
			  $pdf->Cell(50,5,$row["peso"],1,1,'C');
			  $pdf->Cell(50,5,$row["email"],1,0,'C');
			  $pdf->Cell(50,5,$row["celular"],1,0,'C');
			  $pdf->Cell(50,5,$row["direccion"],1,1,'C');
	    ...
	    $pdf->Output("d:\ReporteEmpleados.pdf");  			 
?>